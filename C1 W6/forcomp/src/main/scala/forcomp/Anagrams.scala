...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase.groupBy(c => c).map(c => (c._1, c._2.length)).toList.sortBy(_._1)
  }
    ...
    occurrences match {
      ...
      case (c, n) :: tail => {
        ...
        tailCombinations ++ (for {
          n <- 1 to n
          tailCombinations <- tailCombinations
        } yield (c, n) :: tailCombinations)
      }
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.foldLeft(x.toMap)((acc, elem) => {
      val (char, count) = elem
      val newCount = acc(char) - count
      if (newCount > 0) acc.updated(char, newCount)
      else acc - char
    }).toList.sortBy(_._1)
  }
